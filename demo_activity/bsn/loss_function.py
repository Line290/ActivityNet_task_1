...
def bi_loss_label_smoothing(scores,anchors,opt):
    ...
    loss = coef_1*(pmask-0.3)*pmask*torch.log(anchors+0.00001) + coef_0*(1.0-pmask-0.3)*(1-pmask)*torch.log(1.0-anchors+0.00001)
#     loss = coef_1*pmask*torch.log(anchors+0.00001)*(1 - anchors)**2 + coef_0*(1.0-pmask)*torch.log(1.0-anchors+0.00001)*(anchors)**2
#     loss = 0.25*pmask*torch.log(anchors+1e-12)*(1-anchors)**2
...
def TEM_loss_calc(anchors_action,anchors_start,anchors_end,
    ...
    loss_action,num_sample_action=bi_loss_label_smoothing(match_scores_action,anchors_action,opt)
...
#     loss_end_small,num_sample_end_small=bi_loss(match_scores_end,anchors_end,opt)
    loss_start_small,num_sample_start_small=bi_loss_label_smoothing(match_scores_start,anchors_start,opt)
    loss_end_small,num_sample_end_small=bi_loss_label_smoothing(match_scores_end,anchors_end,opt)
...
def PEM_loss_function(anchors_iou,match_iou,model,opt):
    # append
    match_iou, is_whole_length = match_iou
    
    is_whole_length = is_whole_length.cuda()
    is_whole_length_p = (is_whole_length ==1.0).float() * 0.01
    is_whole_length_n = (is_whole_length <1.0).float()
    is_whole_length = is_whole_length_p + is_whole_length_n
    # finished append
    ...
    # append
    iou_loss = torch.sum(iou_loss * iou_weights * is_whole_length) / torch.sum(iou_weights)
