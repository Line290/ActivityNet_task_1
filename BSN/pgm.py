...
def generateProposals(opt,video_list,video_dict):
    ...
    for video_name in video_list:
        tdf=pandas.read_csv("./output/"+opt["arch"]+opt["fix_scale"]+"_TEM_results/"+video_name+".csv")
        #for different length:
        if opt['fix_scale'] == 'rescale':
            pass
        else:
            tscale = tdf.shape[0]    
            tgap = 1./tscale
...
#         new_props=numpy.stack(new_props)
        # yiheng add for non_scale
        try:
            new_props=numpy.stack(new_props)
        except:
            print('video has no proposal', video_name)
            continue
        # end add
    ...
    #                 gt_xmins.append(max(video_info["annotations"][idx]["segment"][0]/corrected_second, 0.))
    #                 gt_xmaxs.append(max(video_info["annotations"][idx]["segment"][1]/corrected_second, 1.))
            ...
            is_whole_length = []
            for j in range(len(new_df)):
                tmp_new_iou=max(iou_with_anchors(new_df.xmin.values[j],new_df.xmax.values[j], [0.], [1.]))
                is_whole_length.append(float((tmp_new_iou>0.95)&(tmp_new_iou<1.05)))
            new_df['is_whole_length'] = is_whole_length
            # finished append
...
#             print 'success'
            ...
            pass
        new_df.to_csv("./output/"+opt["arch"]+opt["fix_scale"]+"_PGM_proposals/"+video_name+".csv",index=False)
...
def generateFeature(opt,video_list,video_dict):
    ...
    for video_name in video_list:
        adf=pandas.read_csv("./output/"+opt["arch"]+opt["fix_scale"]+"_TEM_results/"+video_name+".csv")
        ...
        pdf=pandas.read_csv("./output/"+opt["arch"]+opt["fix_scale"]+"_PGM_proposals/"+video_name+".csv")
        ...
        numpy.save("./output/"+opt["arch"]+opt["fix_scale"]+"_PGM_feature/"+video_name,feature_bsp)
