...
def video_post_process(opt,video_list,video_dict,all_PEM_results, state):
...
#     print("number of PEM results: ", len(all_fold_pathes))
    for video_name in video_list:
        ...
        df = [all_PEM_results[video_name][i] for i in range(len(state)) if state[i]==1]
...
def BSN_post_processing(opt, all_PEM_results, state):
    ...
    for tid in range(opt["post_process_thread"]-1):
        ...
        p = mp.Process(target = video_post_process,args =(opt,tmp_video_list,video_dict,all_PEM_results, state))
        p.start()
        processes.append(p)
