...
def plot_metric(opt,average_nr_proposals, average_recall, recall, tiou_thresholds=np.linspace(0.5, 0.95, 10)):
    ...
    for idx, tiou in enumerate(tiou_thresholds[::1]):
...
#                 label="tiou=[" + str(tiou) + "], area=" + str(int(area_under_curve[2*idx]*100)/100.), 
        ax.plot(average_nr_proposals, recall[idx,:], color=colors[idx],
                label="tiou=[" + str(tiou) + "], area=" + str(int(area_under_curve[idx]*100)/100.), 
...
#         "./Evaluation/data/activity_net_1_3_new_19934.json",
...
#     plot_metric(opt,uniform_average_nr_proposals_valid, uniform_average_recall_valid, uniform_recall_valid)
    ...
    auc = 100.*float(area_under_curve)/uniform_average_nr_proposals_valid[-1]
    return auc